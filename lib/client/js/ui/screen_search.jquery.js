$.fn.regex_searcher = function(screen) {
  ...
  var finput = dom.find(".screen-filter input");
  var filter_input = dom.find(".filter input");
  ...
  // Update stream, if checkbox is clicked
  ...
  finput.focus(function() {
    ...
    setTimeout(function() {
      ...
      // Only highlight if typing has stopped and regex is valid
      if (((now - dom.data('last_change')) >= (poll_interval-10)) 
          && (val.search(/\S/ig) >= 0) && val != finput.attr("title")) {
        try {
          screen.add_filter(val)
        } catch(err) { dom.addClass("bad-regex"); return; }
        searcher.scroll_next();
      }
    }, poll_interval);
  ...
  this.scroll_next = function() {
    ...
    if (next && next.length) {
      ...
      sconsole.scrollTop(next);
    } else {
      sconsole.scrollTop(0);
    }
  }
  ...
  this.highlight = function(dom_to_highlight) {
    ...
    if (html.length < dom_to_highlight.html().length) {
      ...
      // remove hidden field, if line matches
      dom_to_highlight.removeAttr('hidden');
    } else {
      if (filter_input.is(':checked'))
        // add hidden field, if line not matches and filter checkbox is checked
        dom_to_highlight.hide()
      else
        // remove hidden field, if filter checkbox isn't checked
        dom_to_highlight.show()
    }
  }
  ...
  this.reset_filter = function() {
    dom.removeClass("bad-regex");
    screen.remove_filter();
    result_count.html("");
    sconsole.find(".match").each(function() {
      $(this).removeClass('match').removeClass('active-match')
        .html($(this).data('orig_text'));
        // Show all the hidden elements 
       //$('.console').children("p").show();
       $('.console p').show()
    });
  }
