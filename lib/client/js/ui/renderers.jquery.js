...
Stream.prototype.log = function(log_file, msg) {
  if (!this._paused) {
    /*
	start --> condition to allow filtering logs by regex
    */
    if(this.filter!=null && typeof(this.filter)!="undefined"){
		if(msg.match(this.filter)){
		   ...
		   var cons = this._dom.find(".console");
		   cons.append(html).scrollTop(cons.attr("scrollHeight"));		
		   //this._dom.find(".console").append(html);
		}
    }  /* end --> condition to allow filtering logs by regex*/
    else{
	msg = msg.replace(/</ig,"&lt;").replace(/>/ig,"&gt;");
    	var html = $('<p><span class="label labelcolor'
	+ log_file.color + '">' + log_file.node.label + ':'
